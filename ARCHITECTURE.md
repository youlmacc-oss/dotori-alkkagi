...
- **선공**: 1:1은 **도토리 금화** 시드 동전. `roomId`+`matchGen`+host+guest를 FNV-1a로 접어 짝수=**앞면=호스트(흑)**, 홀수=**뒷면=게스트(백)**. 앞뒤 반반, 양쪽이 같이 계산. 원화 동전·화면 메시 없음. 1인은 항상 호스트(흑). 도토리는 정산만. 봇이 선공이면 사람이 시작 버튼을 대신 누른다. `room_state.firstId`에 선공을 적는다. 다시하기(`matchGen++`)면 다시 던진다.
